...
bool Routeman::DeleteRoute( Route *pRoute )
{
    if( pRoute ) {
        ...
        {
            int ret = OCPNMessageBox( NULL, _("You are trying to delete an active AIS MOB route, are you REALLY sure?"), _("¾¯¸æ"), wxYES_NO );
...
void Routeman::DeleteTrack( Route *pRoute )
{
    if( pRoute ) {
        ...
        if( count > 10000) {
            pprog = new wxProgressDialog( _("É¾³ýº½¼£"), _T("0/0"), count, NULL, 
