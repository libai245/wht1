...
//--------------------------------------------------------------------
void TriangleDraw(wxPoint point,int len,double angle,wxPoint out_point[3])
{
	/*out_point[0].x = point.x + (len/sqrt((double)3)) * sin (angle / 180 * PI);
	out_point[0].y = point.y - (len/sqrt((double)3)) * cos (angle / 180 * PI);
	out_point[1].x = point.x + (len/sqrt((double)3)) * sin ((angle + 120) / 180 * PI);
	out_point[1].y = point.y - (len/sqrt((double)3)) * cos ((angle + 120) / 180 * PI);
	out_point[2].x = point.x + (len/sqrt((double)3)) * sin ((angle + 240) / 180 * PI);
	out_point[2].y = point.y - (len/sqrt((double)3)) * cos ((angle + 240) / 180 * PI);*/

	out_point[0].x = point.x + len * (cos(20. / 180 * PI) - 0.5 * sin(20. / 180 * PI) * tan(35. / 180 * PI) ) * sin(angle / 180 * PI);
	out_point[0].y = point.y - len * (cos(20. / 180 * PI) - 0.5 * sin(20. / 180 * PI) * tan(35. / 180 * PI) ) * cos(angle / 180 * PI);
	out_point[1].x = point.x + 0.5 * len * sin(20. / 180. * PI) / cos(35. / 180 * PI) * sin ((angle + 125) / 180 * PI);
	out_point[1].y = point.y - 0.5 * len * sin(20. / 180. * PI) / cos(35. / 180 * PI) * cos ((angle + 125) / 180 * PI);
	out_point[2].x = point.x + 0.5 * len * sin(20. / 180. * PI) / cos(35. / 180 * PI) * sin ((angle + 235) / 180 * PI);
	out_point[2].y = point.y - 0.5 * len * sin(20. / 180. * PI) / cos(35. / 180 * PI) * cos ((angle + 235) / 180 * PI);
}

void RADARDraw(Radar_Target_Data td,ocpnDC& dc )
{
	if (!g_bShowRadar)	return;
	if(td.Lat[0] == 0 && td.Lon[0] == 0) return;
		
	wxPen target_pen( wxColor(88,174,0), 2,wxSOLID );
	wxBrush target_brush = wxBrush( wxColor(255,0,0) );
	...
	cc1->GetCanvasPointPix( td.Lat[i], td.Lon[i], &TargetPoint );
	//dc.TriangleDraw(TargetPoint.x,TargetPoint.y );
	wxPoint out_point[3];
	TriangleDraw(TargetPoint,40,td.angle[i],out_point);               //得到三角形三个点

	
	dc.DrawPolygon(3,out_point);                                //绘制多边形
	//dc.DrawCircle(37.5 , 122.2 , 10);
	}
}

void TextDraw(ocpnDC& dc)
{
	wxPen target_pen( wxColor(88,174,0), 2,wxSOLID );
	/*wxBrush target_brush = wxBrush( wxColor(255,0,0) );
	//wxPoint m_pBorder1;
	ArePoints TargerAre[50];


	TargerAre[0].lat = 37.569715;
	TargerAre[0].lon = 122.085901;

	TargerAre[1].lat = 37.554003;
	TargerAre[1].lon = 122.085901;

	TargerAre[2].lat = 37.554546;
	TargerAre[2].lon = 122.11107;

	TargerAre[3].lat = 37.569783;
	TargerAre[3].lon = 122.109703;
	//m_pBorder1.x = 37.565100;
	//m_pBorder1.y = 122.098915;
	dc.SetPen( target_pen );
	//dc.SetBrush( target_brush );

	wxPoint TargetPoint[4];
	for(int i = 0;i <= 3;i++)
	{

		cc1->GetCanvasPointPix( TargerAre[i].lat, TargerAre[i].lon, &TargetPoint[i] );
	}
	for (int j = 0;j <= 2;j++)
	{
		dc.StrokeLine(TargetPoint[j],TargetPoint[j+1]);
	}
	dc.StrokeLine(TargetPoint[3],TargetPoint[0]);*/
	...
	//wxPen target_pen( wxColor(255,255,255), 1,wxSOLID );
	//wxBrush target_brush = wxBrush( wxColor(255,0,0) );
	...
	wxString ssb = wxT("哈尔滨工业大学(威海)");
	wxString ssc = wxT("麻子山");

	wxString ssd = wxT("远遥");

	wxPoint TargetPoint1;
	//struct radarTarget target2 = target;
	AreaText areatext[20];
	areatext[0].TextLat = 37.561645;
	areatext[0].TextLon = 122.095663;

	areatext[1].TextLat = 37.57;
	areatext[1].TextLon = 122.08;

	areatext[2].TextLat = 37.526988;
	areatext[2].TextLon = 122.072107;

	areatext[3].TextLat = 37.536981;
	areatext[3].TextLon = 122.053218;

	areatext[4].TextLat = 37.541859;
	areatext[4].TextLon = 122.087218;


	cc1->GetCanvasPointPix( areatext[0].TextLat, areatext[0].TextLon, &TargetPoint1 );
	dc.DrawText(ssa,TargetPoint1.x,TargetPoint1.y);
	cc1->GetCanvasPointPix( areatext[1].TextLat, areatext[1].TextLon, &TargetPoint1 );
	dc.DrawText(wxT("褚岛"),TargetPoint1.x,TargetPoint1.y);

	cc1->GetCanvasPointPix( areatext[2].TextLat, areatext[2].TextLon, &TargetPoint1 );
	dc.DrawText(ssb,TargetPoint1.x,TargetPoint1.y);

	cc1->GetCanvasPointPix( areatext[3].TextLat, areatext[3].TextLon, &TargetPoint1 );
	dc.DrawText(ssc,TargetPoint1.x,TargetPoint1.y);

	cc1->GetCanvasPointPix( areatext[4].TextLat, areatext[4].TextLon, &TargetPoint1 );
	dc.DrawText(ssd,TargetPoint1.x,TargetPoint1.y);
