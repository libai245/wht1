...
//----------------------------------------------------
extern Radar_Target_Data pRadarData;
//-----------------------------------------hitwh wht 2015.10.16
...
void ChartCanvas::OnRolloverPopupTimerEvent( wxTimerEvent& event )
{
	...
	wxPoint TargetPoint1;
	cc1->GetCanvasPointPix( pRadarData.Lat[0], pRadarData.Lon[0], &TargetPoint1 );
	if (m_myLat < TargetPoint1.x +10  && m_myLat > TargetPoint1.x -10 && m_myLon < TargetPoint1.x + 10 && m_myLon < TargetPoint1.x - 10)
	{
		wxString s;
		wxString lon1,lat1;
		lat1 << pRadarData.Lat[0];
		lon1 << pRadarData.Lon[0];
    ...
    if( g_pAIS && g_pAIS->GetNumTargets() && g_bShowAIS ) {
        SelectItem *pFind = pSelectAIS->FindSelection( m_cursor_lat, m_cursor_lon,//判断鼠标是否移到了AIS目标
                                                       SELTYPE_AISTARGET );
        if( pFind ) {
        ...
        while( node ) {
            ...
            if( pr && pr->IsVisible() ) {
                ...
                if( !m_pRouteRolloverWin->IsActive() ) {
                    ...
                    m_pRouteRolloverWin->SetString( s );//通过这个函数来是、设置航线的信息提示框    wht
...
//            m_zoom_target =  VPoint.chart_scale / factor;
        ...
        DoZoomCanvas( factor );//factor 是因数   表示放大或者缩小
    }
        ...
        if(!g_btouch){
            if( parent_frame->nRoute_State )                  // creating route?
            {
                        ...
                        && !pNearbyPoint->m_bIsInTrack && !pNearbyPoint->m_bIsInLayer )
                {
                    int dlg_return;
    #ifndef __WXOSX__
                    dlg_return = OCPNMessageBox( this, _("用最近的航点?"),
                                                    _("创建航线"),
                ...
                if(m_pMouseRoute){
                    if( parent_frame->nRoute_State == 1 ) {
                        ...
                        if( m_pMouseRoute->m_NextLegGreatCircle ) {
                            ...
                            int answer = OCPNMessageBox( this, msg, _("创建航线"), wxYES_NO | wxNO_DEFAULT );
        ...
        if(g_btouch) {
            ...
            if( parent_frame->nRoute_State )                  // creating route?
            {
                    ...
                    && !pNearbyPoint->m_bIsInTrack && !pNearbyPoint->m_bIsInLayer )
                {
                    int dlg_return;
                    #ifndef __WXOSX__
                    dlg_return = OCPNMessageBox( this, _("使用附近的航点?"),
                                                _("创建航线"),
                                                (long) wxYES_NO | wxCANCEL | wxYES_DEFAULT );
                                                #else
                                                dlg_return = wxID_YES;
...
#ifdef __WXMSW__
    wxFont *qFont = GetOCPNScaledFont(_T("菜单"));
    item->SetFont(*qFont);
#endif
    menu->Prepend(item);
}
...
void MenuAppend( wxMenu *menu, int id, wxString label)
{
    wxMenuItem *item = new wxMenuItem(menu, id, label);
#ifdef __WXMSW__
    wxFont *qFont = GetOCPNScaledFont(_("菜单"));
    item->SetFont(*qFont);
#endif
    menu->Append(item);
}
...
void ChartCanvas::CanvasPopupMenu( int x, int y, int seltype )
{
    ...
    wxMenu* menuWaypoint = new wxMenu( _("航点") );
    wxMenu* menuRoute = new wxMenu( _("航线") );
    wxMenu* menuTrack = new wxMenu( _("航迹") );
...
//    gtk_widget_modify_bg (GTK_WIDGET(contextMenu->m_menu), GTK_STATE_NORMAL, &color);
    ...
    if( ! m_pMouseRoute ) {
        if( m_bMeasure_Active )
            MenuPrepend( contextMenu, ID_DEF_MENU_DEACTIVATE_MEASURE, _menuText( _("停止测量"), _T("Esc") ) );
        else
            MenuPrepend( contextMenu, ID_DEF_MENU_ACTIVATE_MEASURE, _menuText( _( "测量" ), _T("M") ) );
//            contextMenu->Prepend( ID_DEF_MENU_ACTIVATE_MEASURE, _menuText( _( "Measure" ), _T("F4") ) );
    ...
    if( !VPoint.b_quilt ) {
        if( parent_frame->GetnChartStack() > 1 ) {
            MenuAppend( contextMenu, ID_DEF_MENU_MAX_DETAIL, _( "最多细节信息量" ) );
            MenuAppend( contextMenu, ID_DEF_MENU_SCALE_IN, _menuText( _( "缩小" ), _T("Ctrl-Left") ) );
            MenuAppend( contextMenu, ID_DEF_MENU_SCALE_OUT, _menuText( _( "放大" ), _T("Ctrl-Right") ) );
        }

        if( ( Current_Ch && ( Current_Ch->GetChartFamily() == CHART_FAMILY_VECTOR ) ) || ais_areanotice ) {
            MenuAppend( contextMenu, ID_DEF_MENU_QUERY, _( "对象查询..." ) );
        }
    ...
    MenuAppend( contextMenu, ID_DEF_MENU_DROP_WP, _menuText( _( "标记此处" ), _T("Ctrl-M") ) );

    if( !bGPSValid ) MenuAppend( contextMenu, ID_DEF_MENU_MOVE_BOAT_HERE, _( "将船移到此处" ) );

    if( !( g_pRouteMan->GetpActiveRoute() || ( seltype & SELTYPE_MARKPOINT ) ) )
        MenuAppend( contextMenu, ID_DEF_MENU_GOTO_HERE, _( "航行至此处" ) );

    MenuAppend( contextMenu, ID_DEF_MENU_GOTOPOSITION, _("中心视图...") );

    if( !g_bCourseUp ) MenuAppend( contextMenu, ID_DEF_MENU_COGUP, _("船头向上模式") );
    else {
        if( !VPoint.b_quilt && Current_Ch && ( fabs( Current_Ch->GetChartSkew() ) > .01 )
            && !g_bskew_comp ) MenuAppend( contextMenu, ID_DEF_MENU_NORTHUP, _("海图向上模式") );
        else
            MenuAppend( contextMenu, ID_DEF_MENU_NORTHUP, _("北方向上模式") );
    }

    bool full_toggle_added = false;
    if(g_btouch){
        MenuAppend( contextMenu, ID_DEF_MENU_TOGGLE_FULL, _("开启全屏") );
    ...
    if( pasteBuffer != KML_PASTE_INVALID ) {
        switch( pasteBuffer ) {
            case KML_PASTE_WAYPOINT: {
                MenuAppend( contextMenu, ID_PASTE_WAYPOINT, _( "粘贴航点" ) );
                break;
            }
            case KML_PASTE_ROUTE: {
                MenuAppend( contextMenu, ID_PASTE_ROUTE, _( "粘贴航线" ) );
                break;
            }
            case KML_PASTE_TRACK: {
                MenuAppend( contextMenu, ID_PASTE_TRACK, _( "粘贴航迹" ) );
                break;
            }
    ...
    if( ( VPoint.b_quilt ) && ( pCurrentStack && pCurrentStack->b_valid ) ) {
        ...
        if( dbIndex != -1 )
            MenuAppend( contextMenu, ID_DEF_MENU_QUILTREMOVE, _( "隐藏这个海图" ) );
    }
    ...
    //  ChartGroup SubMenu
    wxMenuItem* subItemChart = contextMenu->AppendSubMenu( subMenuChart, _("海图组") );
    ...
    if( g_pGroupArray->GetCount() ) {
          ...
          wxMenuItem* subItem1 = subMenuChart->AppendRadioItem( wxID_CANCEL , _T("临时") );
    ...
    if( g_pAIS ) {
        MenuAppend( contextMenu, ID_DEF_MENU_AISTARGETLIST, _("AIS目标列表...") );

        if( seltype & SELTYPE_AISTARGET ) {
            MenuAppend( menuAIS, ID_DEF_MENU_AIS_QUERY, _( "目标查询..." ) );
            ...
            MenuAppend( menuAIS, ID_DEF_MENU_AISTARGETLIST, _("目标列表...") );
            if ( 1 /*g_bAISShowTracks*/ ) {
                if( myptarget && myptarget->b_show_track )
                    MenuAppend( menuAIS, ID_DEF_MENU_AISSHOWTRACK, _("隐藏目标轨迹") );
                else
                    MenuAppend( menuAIS, ID_DEF_MENU_AISSHOWTRACK, _("现实目标轨迹") );
            }
            menuFocus = menuAIS;
        }
    }
    ...
    if( seltype & SELTYPE_ROUTESEGMENT ) {
        ...
        if( blay ){
            ...
            MenuAppend( menuRoute, ID_RT_MENU_PROPERTIES, _( "属性..." ) );
            if( m_pSelectedRoute ) {
                if( m_pSelectedRoute->IsActive() ) {
                    ...
                    if( ( indexActive + 1 ) <= m_pSelectedRoute->GetnPoints() ) {
                        MenuAppend( menuRoute, ID_RT_MENU_ACTNXTPOINT, _( "激活下一个航点" ) );
                    }
                    MenuAppend( menuRoute, ID_RT_MENU_DEACTIVATE, _( "使无效" ) );
                }
                else {
            ...
            if( m_pSelectedRoute ) {
                if( m_pSelectedRoute->IsActive() ) {
                    ...
                    MenuAppend( menuRoute, ID_RT_MENU_ACTIVATE, _( "激活" ) );
                }
            }
            MenuAppend( menuRoute, ID_RT_MENU_INSERT, _( "插入航路点" ) );
            MenuAppend( menuRoute, ID_RT_MENU_APPEND, _( "附加航路点" ) );
            MenuAppend( menuRoute, ID_RT_MENU_COPY, _( "复制为..." ) );
            MenuAppend( menuRoute, ID_RT_MENU_DELETE, _( "删除..." ) );
            MenuAppend( menuRoute, ID_RT_MENU_REVERSE, _( "倒转..." ) );
    ...
    if( seltype & SELTYPE_TRACKSEGMENT ) {
        ...
        if( blay ) {
            delete menuTrack;
            menuTrack = new wxMenu( _("航迹层") );
            MenuAppend( menuTrack, ID_TK_MENU_PROPERTIES, _( "属性..." ) );
        }
        else {
            MenuAppend( menuTrack, ID_TK_MENU_PROPERTIES, _( "属性..." ) );
            MenuAppend( menuTrack, ID_TK_MENU_COPY, _( "复制为KML" ) );
            MenuAppend( menuTrack, ID_TK_MENU_DELETE, _( "删除..." ) );
        }
    ...
    if( seltype & SELTYPE_ROUTEPOINT ) {
        ...
        if( blay ){
            ...
            MenuAppend( menuWaypoint, ID_WP_MENU_PROPERTIES, _( "属性..." ) );

            if( m_pSelectedRoute && m_pSelectedRoute->IsActive() )
                MenuAppend( menuWaypoint, ID_RT_MENU_ACTPOINT, _( "激活" ) );
        }
        else {
            MenuAppend( menuWaypoint, ID_WP_MENU_PROPERTIES, _( "属性..." ) );
            if( m_pSelectedRoute && m_pSelectedRoute->IsActive() ) {
            ...
            if( m_pSelectedRoute && m_pSelectedRoute->IsActive() ) {
                if(m_pSelectedRoute->m_pRouteActivePoint == m_pFoundRoutePoint ) {
                    int indexActive = m_pSelectedRoute->GetIndexOf( m_pSelectedRoute->m_pRouteActivePoint );
                    if( ( indexActive + 1 ) <= m_pSelectedRoute->GetnPoints() )
                        MenuAppend( menuWaypoint, ID_RT_MENU_ACTNXTPOINT, _( "激活下一个航点" ) );
                }
            }
            if( m_pSelectedRoute->GetnPoints() > 2 )
                MenuAppend( menuWaypoint, ID_RT_MENU_REMPOINT, _( "从航线中移除" ) );

            MenuAppend( menuWaypoint, ID_WPT_MENU_COPY, _( "复制为KML" ) );

            if( m_pFoundRoutePoint->GetIconName() != _T("mob") )
                MenuAppend( menuWaypoint, ID_RT_MENU_DELPOINT,  _( "删除" ) );

            wxString port = FindValidUploadPort();
            m_active_upload_port = port;
            wxString item = _( "发送到GPS" );
            if( !port.IsEmpty() ) {
                item.Append( _T(" ( ") );
                item.Append( port );
                item.Append(_T(" )") );
    ...
    if( seltype & SELTYPE_MARKPOINT ) {
        ...
        if( blay ){
            ...
            if( !g_pRouteMan->GetpActiveRoute() )
                MenuAppend( menuWaypoint, ID_WP_MENU_GOTO, _( "航行至此" ) );

            MenuAppend( menuWaypoint, ID_WPT_MENU_COPY, _( "复制为KML" ) );

            if( m_pFoundRoutePoint->GetIconName() != _T("mob") )
                MenuAppend( menuWaypoint, ID_WP_MENU_DELPOINT, _( "删除" ) );

            wxString port = FindValidUploadPort();
            m_active_upload_port = port;
            wxString item = _( "发送到 GPS" );
            if( !port.IsEmpty() ) {
                item.Append( _T(" ( ") );
                item.Append( port );
                item.Append(_T(" )") );
            ...
            if( ( m_pFoundRoutePoint == pAnchorWatchPoint1 ) || ( m_pFoundRoutePoint == pAnchorWatchPoint2 ) )
                MenuAppend( menuWaypoint, ID_WP_MENU_CLEAR_ANCHORWATCH, _( "清除锚位监控" ) );
            else {
                if( !( m_pFoundRoutePoint->m_bIsInLayer )
                    ...
                    if( dist * 1852. <= g_nAWMax )
                        MenuAppend( menuWaypoint,  ID_WP_MENU_SET_ANCHORWATCH,  _( "设置锚位监控" ) );
                }
            }
        }
    ...
    if( seltype & SELTYPE_TIDEPOINT ){
        ...
        bsep = true;
        MenuAppend( menuFocus, ID_DEF_MENU_TIDEINFO, _( "显示隐藏信息" ) );
    }

    if( seltype & SELTYPE_CURRENTPOINT ) {
        if( !bsep )
            menuFocus->AppendSeparator();
        MenuAppend( menuFocus,  ID_DEF_MENU_CURRENTINFO, _( "显示潮流信息" ) );
    }
...
//                        WarpPointer(popx,popy);
    ...
    case ID_RT_MENU_DELETE: {
        ...
        if( g_bConfirmObjectDelete ) {
            dlg_return = OCPNMessageBox( this,  _("Are you sure you want to delete this route?"),
                _("删除海图航线"), (long) wxYES_NO | wxCANCEL | wxYES_DEFAULT );
        }
    ...
    case ID_TK_MENU_DELETE: {
        int dlg_return = wxID_YES;
        if( g_bConfirmObjectDelete ) {
            dlg_return = OCPNMessageBox( this, _("确定删除航迹?"),
                _("删除航迹"), (long) wxYES_NO | wxCANCEL | wxYES_DEFAULT );
        }
...
//------------------------------------------------------
void ChartCanvas::ShowAISTargetList2( void )
{
	wxWindow *pwin = wxDynamicCast(this, wxWindow);
	MyAISlistDialog* AISlist = new MyAISlistDialog(pwin,g_pauimgr);
	cc1->ReloadVP();
}
//--------------------------------------------------hitwh wht 2015.10.12
...
emboss_data *ChartCanvas::EmbossDepthScale()
{
    ...
    ped->x = ( GetVP().pix_width - ped->width );//浮标的屏幕横坐标位置
    ped->y = 40;								//浮标的屏幕纵坐标位置
    return ped;
}
...
void ChartCanvas::CreateDepthUnitEmbossMaps( ColorScheme cs )
{
    ...
    if( style->embossFont == wxEmptyString )
        font = wxFont( 30, wxFONTFAMILY_ROMAN, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_BOLD );//字体的设置
    else
...
// Create the 3 DepthUnit emboss map structures
    ...
    m_pEM_Meters = CreateEmbossMapData( font, emboss_width, emboss_height, _("哈尔滨工业大学（威海）"), cs );
        ...
        const wxChar *str, ColorScheme cs )
{
    ...
    //  Paint on it
    temp_dc.SetBackground( *wxWHITE_BRUSH );//*wxWHITE_BRUSH
	temp_dc.SetTextBackground( *wxWHITE );//*wxWHITE
