...
enum
{
	...
	//-----------------------------------------------
	ID_CLOSECHART,
	ID_MENU_RADAR_TARGETS,
	ID_MENU_MYAIS_TARGETLIST,//-------------2015.10.12
	//----------------------------------hitwh wht ¹Ø±Õº£Í¼°´Å¥
    ID_MENU_MARK_CURSOR,
    ID_MENU_MARK_MOB,
...
class MyFrame: public wxFrame
{
  public:
	...
	//---------------------------------------------------------
	void OnDecodeRadarMsg(wxString& msg);
...
extern struct Radar_Target_Data
{
	double Lat[1000];
	double Lon[1000];
	double angle[1000];
...
extern struct AreaText
{
	double TextLat;
	double TextLon;
}s2;

extern struct ArePoints
{
	double lon;
	double lat;
}s3;
